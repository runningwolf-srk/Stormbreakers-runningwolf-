        ...
        <div className="mt-10 space-y-10">
          {relics.map(r=>(
            <div key={r.n} className="bg-[#0a0a0a] border border-amber-900/20">
              <div className="p-5 border-b border-zinc-800/50"><p className="text-[10px] text-zinc-600">RELIC {r.n}</p><h3 className="font-bold text-xl mt-1">{r.icon} {r.t}</h3><p className="text-[11px] text-amber-700/60 mt-1">{r.v}</p></div>
              <div className="relative aspect-video bg-black"><iframe className="absolute inset-0 w-full h-full" src={`https://www.youtube.com/embed/${r.id}`} title={r.t} frameBorder="0" allowFullScreen></iframe></div>
            </div>
          ))}
        </div>
        <div className="mt-10 space-y-3">
          {sealed.map(s=>(
            <div key={s.n} className="bg-black border border-zinc-800 p-4 flex justify-between items-center opacity-40"><p className="text-[11px]">RELIC {s.n} — 🔒 {s.t}</p><p className="text-[10px] italic text-zinc-600">Sealed until the next page is revealed.</p></div>
          ))}
        </div>
