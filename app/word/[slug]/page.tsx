...
export default function WordDetail({params}:{params:{slug:string}}) {
  ...
  if (!r) {
    return (
      <main className="px-4 py-24 max-w-2xl mx-auto text-center">
        ...
        <Link href="/word" className="inline-block mt-8 px-5 py-3 bg-white text-black rounded-full font-bold text-sm">
          ← Back to Word
        </Link>
      </main>
    )
  ...
  return (
    <main className="px-4 py-12 max-w-3xl mx-auto">
      <Link href="/word" className="text-xs uppercase tracking-widest text-zinc-500">
        ← Back to Word
      </Link>
      ...
      <div className="mt-8 bg-amber-950/10 border border-amber-900/20 rounded-2xl p-6">
        <p className="text-xs uppercase tracking-widest text-amber-500">
          Foundation — What it meant then / What it reveals about God / How it points to Christ
        </p>
        <p className="mt-3 text-sm text-zinc-300">Write 400w study here...</p>
      </div>
    </main>
  )
}
