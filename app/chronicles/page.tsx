    ...
    <main className="px-4 py-12 max-w-5xl mx-auto">
      ...
      {acts.map((act) => (
        <div key={act.id} className="mt-12">
          <h2 className="font-black text-sm uppercase tracking-[0.3em] text-zinc-400">
            ACT {act.id} — {act.name}
          </h2>
          <p className="text-xs text-zinc-600">{act.desc}</p>
          <div className="grid md:grid-cols-2 gap-4 mt-4">
            {CANON.filter((r:any) => r.act === act.id).map((r:any) => (
              <Link key={r.slug} href={`/chronicles/${r.slug}`} href={`/chronicles/${r.slug}`}>
                <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 hover:border-amber-500/30 h-full">
                  <p className="text-xs uppercase" style={{color:r.color}}>
                    {r.symbol} {r.title} • {r.scriptureRef}
                  </p>
